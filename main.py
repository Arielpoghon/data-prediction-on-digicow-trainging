...
def preprocess(train, test):
    ...
    vec = TfidfVectorizer(max_features=200, ngram_range=(1,2), stop_words='english', min_df=2)
...
def train_stack_model(train_proc, y):
    ...
    oof_preds = np.zeros((train_proc.shape[0], 4)) # 4 base models now
    base_models = [
        XGBClassifier(n_estimators=1000, max_depth=6, learning_rate=0.02,
                      subsample=0.75, colsample_bytree=0.75, use_label_encoder=False,
                      eval_metric='logloss', random_state=42, tree_method='hist',
                      scale_pos_weight=pos_weight),
        LGBMClassifier(n_estimators=1200, learning_rate=0.02, num_leaves=50,
                       subsample=0.75, colsample_bytree=0.75, verbose=-1, random_state=42,
