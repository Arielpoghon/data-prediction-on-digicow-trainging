"""
DigiCow Final Submission
Session peer feature: 96% train / 86% test coverage confirmed.
"""
...
def parse_first(s):
    """Extract first trainer token regardless of list formatting."""
    parts = parse_list(s)
...
def build_features(train, test, prior):
    ...
    # Session key: use parse_first which handles both plain and list formats
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # 1. SESSION PEER ADOPTION — from Prior only, zero leakage
    #    96% train / 86% test coverage confirmed
    # ══════════════════════════════════════════════════════════════════════════
    ...
    for df in [train, test]:
        df.drop(columns=[c for c in df.columns if c.startswith('sess_')],
                inplace=True, errors='ignore')
        merged = df[['sk']].merge(sess, on='sk', how='left')
        for col in ['sess_07_mean','sess_90_mean','sess_120_mean',
            ...
            df[col] = merged[col].fillna(fill).values

    n_tr = (train['sess_n'] > 0).sum()
    n_te = (test['sess_n'] > 0).sum()
    print(f"  Session matches — train:{n_tr}/{len(train)}  test:{n_te}/{len(test)}")

    # ══════════════════════════════════════════════════════════════════════════
    # 2. PER-FARMER HISTORY — past sessions only (strict <)
    # ══════════════════════════════════════════════════════════════════════════
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # 3. TRAINER adoption rates (Bayesian-smoothed)
    # ══════════════════════════════════════════════════════════════════════════
    ...
    for df in [train, test]:
        for attr, gm in [('tr07',global07),('tr90',global90),('tr120',global120)]:
            period = attr.replace('tr','')
            cname  = f'trainer_{period.zfill(2) if period.isdigit() else period}_rate'
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # 4. TOPIC adoption rates (Bayesian-smoothed)
    # ══════════════════════════════════════════════════════════════════════════
    ...
    for tgt in TARGETS:
        ...
        for df in [train, test]:
            ...
            df[f'trainer_topic_{period}'] = (df[f'topic_{period}_rate'] *
                                              df.get(f'trainer_{period}_rate', gm))

    # ══════════════════════════════════════════════════════════════════════════
    # 5. GROUP adoption rates (Bayesian-smoothed)
    # ══════════════════════════════════════════════════════════════════════════
    if 'group_name' in prior.columns:
...
def preprocess(train, test):
    ...
    for df in [train, test]:
        for period in ['07','90','120']:
            ...
            sm  = df.get(f'sess_{period}_mean', pd.Series(0, index=df.index))
            ...
            # Session interactions — the new signal
            df[f'sess_x_trainer_{period}']    = sm * tr
            df[f'sess_x_topic_{period}']      = sm * tp
            df[f'sess_x_prior_{period}']      = sm * pa
            df[f'sess_x_group_{period}']      = sm * grp
...
def main():
    ...
    train, test = build_features(train, test, prior)
